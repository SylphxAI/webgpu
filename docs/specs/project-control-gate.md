...
- generated `.groundatlas*` outputs plus JSON/Markdown GroundAtlas reports are
  evidence/navigation read models only;
...
3. run `SylphxAI/groundatlas@v0.1.3` with `package-spec:
...
5. assert and upload the manifest report, fleet JSON report, and Markdown fleet
   scorecard as `groundatlas-package-dogfood`;
...
- `ga fleet --require-atlas --strict` emits a Markdown scorecard with the
  GroundAtlas title and summary `1 adopted, 0 warning, 0 blocked, 1 total`.
