...
- a single CI `groundatlas` job using `SylphxAI/groundatlas@v0.1.3` with
  `groundatlas@0.1.3`;
- assertions that GroundAtlas selects `project.manifest.json`, reports
  ...
  warnings/blockers;
- assertions that the generated Markdown fleet scorecard exists and includes
  the GroundAtlas title plus adopted fleet summary;
...
- Generated `.groundatlas*` reports and JSON/Markdown GroundAtlas reports remain
